      ...
      <div className="text-white flex flex-col items-center pt-10 md:pt-20 px-2 w-full">
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-14">
          {zodiacSigns.map((src, index) => (
            <div
              key={index}
              className="relative flex justify-center items-center w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-lg overflow-hidden border-2 border-blue-400 animate-glow-blue"
              style={{
                boxShadow: "0 0 15px 4px rgba(255, 221, 0, 0.6), 0 0 25px 6px rgba(255, 221, 0, 0.4)",
              }}
            >
      ...
      <div className="mt-16 flex items-center justify-center w-full">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-6xl w-full px-4">
          {features.map((feature, index) => (
            <div
              key={index}
              className="bg-[#0D0F2B] text-white p-6 rounded-2xl shadow-lg border border-white text-center transform transition-transform duration-300 hover:scale-105"
              style={{
